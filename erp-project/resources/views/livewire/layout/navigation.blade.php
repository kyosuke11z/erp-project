...
<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
                    ...
                    @can('view finance')
                        <div class="relative flex items-center">
                            <x-dropdown align="left" width="48">
                                ...
                                <x-slot name="content">
                                    <x-dropdown-link :href="route('finance.index')" wire:navigate>{{ __('รายการการเงิน') }}</x-dropdown-link>
                                    <x-dropdown-link :href="route('finance.categories.index')" wire:navigate>{{ __('จัดการหมวดหมู่') }}</x-dropdown-link>
                                    <x-dropdown-link :href="route('finance.report')" wire:navigate>{{ __('รายงานสรุป') }}</x-dropdown-link>
                                </x-slot>
                            </x-dropdown>
                        </div>
                    @endcan
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @can('user-list')
                <x-responsive-nav-link :href="route('users.index')" :active="request()->routeIs('users.*')">{{ __('จัดการผู้ใช้งาน') }}</x-responsive-nav-link>
            @endcan
            @can('category-list')
                <x-responsive-nav-link :href="route('categories.index')" :active="request()->routeIs('categories.index')" wire:navigate>{{ __('หมวดหมู่') }}</x-responsive-nav-link>
            @endcan
            @can('product-list')
                <x-responsive-nav-link :href="route('products.index')" :active="request()->routeIs('products.index')" wire:navigate>{{ __('สินค้า') }}</x-responsive-nav-link>
            @endcan
            @can('customer-list')
                <x-responsive-nav-link :href="route('customers.index')" :active="request()->routeIs('customers.index')" wire:navigate>{{ __('ลูกค้า') }}</x-responsive-nav-link>
            @endcan
            @can('supplier-list')
                <x-responsive-nav-link :href="route('suppliers.index')" :active="request()->routeIs('suppliers.*')" wire:navigate>{{ __('ซัพพลายเออร์') }}</x-responsive-nav-link>
            @endcan
            @can('purchase-order-list')
                <x-responsive-nav-link :href="route('purchase-orders.index')" :active="request()->routeIs('purchase-orders.*')" wire:navigate>{{ __('ใบสั่งซื้อ') }}</x-responsive-nav-link>
            @endcan
            @can('sales-order-list')
                <x-responsive-nav-link :href="route('sales.index')" :active="request()->routeIs('sales.*')" wire:navigate>{{ __('คำสั่งขาย') }}</x-responsive-nav-link>
            @endcan
            @role('Admin')
                <x-responsive-nav-link :href="route('settings.index')" :active="request()->routeIs('settings.*')" wire:navigate>{{ __('ตั้งค่า') }}</x-responsive-nav-link>
            @endrole
        </div>
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            @can('view finance')
                ...
                <div class="mt-3 space-y-1">
                    <x-responsive-nav-link :href="route('finance.index')" wire:navigate>{{ __('รายการการเงิน') }}</x-responsive-nav-link>
                    <x-responsive-nav-link :href="route('finance.categories.index')" wire:navigate>{{ __('จัดการหมวดหมู่') }}</x-responsive-nav-link>
                    <x-responsive-nav-link :href="route('finance.report')" wire:navigate>{{ __('รายงานสรุป') }}</x-responsive-nav-link>
                </div>
            @endcan
